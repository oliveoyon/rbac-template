...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2>Permissions</h2>
        <button class="btn btn-primary" id="addPermissionBtn"><i class="fas fa-plus"></i> Add Permission</button>
    </div>
    ...
    <table class="table table-striped" id="permissionsTable">
        ...
        <tbody>
            ...
            <tr id="permission-{{ $permission->id }}">
                ...
                <td>
                    <button class="btn btn-sm btn-info editBtn" data-id="{{ $permission->id }}"><i class="fas fa-edit"></i></button>
                    <button class="btn btn-sm btn-danger deleteBtn" data-id="{{ $permission->id }}"><i class="fas fa-trash"></i></button>
                </td>
            </tr>
            @endforeach
        </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        fetch(url, {
            ...
            body: JSON.stringify({ name: nameInput.value, group_id: groupSelect.value })
        })
        .then(async res => {
            if(res.status === 422) {
                const data = await res.json();
                if(data.errors.name){ nameError.textContent = data.errors.name[0]; nameInput.classList.add('is-invalid'); }
                if(data.errors.group_id){ groupError.textContent = data.errors.group_id[0]; groupSelect.classList.add('is-invalid'); }
        ...
        .then(data => {
            if(data){
                ...
                if(id){ document.getElementById(rowId).outerHTML = rowHtml; }
                else{ document.querySelector('#permissionsTable tbody').insertAdjacentHTML('beforeend', rowHtml); }
                modal.hide();
        ...
        if(e.target.closest('.deleteBtn')){
            ...
            Swal.fire({
                ...
                if(result.isConfirmed){
                    fetch(`/admin/permissions/${id}`, { method:'DELETE', headers:{ 'X-CSRF-TOKEN': token } })
                    .then(res=>res.json())
                    .then(data=>{
