            ...
            <div class="card-body">
                <div class="btn-group" role="group">
                    @foreach($roles as $role)
                        ...
                        <label class="btn btn-outline-success me-1 mb-1" for="role-{{ $role->id }}">{{ $role->name }}</label>
                    @endforeach
                </div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            @foreach($group->permissions as $permission)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input permission-checkbox group-{{ $group->id }}" type="checkbox" name="permissions[]" value="{{ $permission->name }}" {{ isset($userPermissions) && in_array($permission->name, $userPermissions) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCount(groupId){
        const count = document.querySelectorAll('.group-' + groupId + ':checked').length;
    ...
    document.querySelectorAll('.select-all').forEach(function(toggle) {
        toggle.addEventListener('change', function() {
            ...
            document.querySelectorAll('.group-' + groupId).forEach(cb => cb.checked = checked);
            updateCount(groupId);
        });
    });
